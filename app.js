...
// ---------- SEND ----------
async function sendEmail(inbox, lead, template) {
...
// ---------- RETRY ----------
async function sendWithRetry(inbox, lead, template) {

  let tries = 0;

  while (tries < 3) {
    try {
      await sendEmail(inbox, lead, template);
      return true;
    } catch (e) {
      tries++;
    }
...
app.post("/run", async (req, res) => {
  ...
  const batch = req.body.batch || leads.length;
  ...
  while (state.index < leads.length && sent < batch) {
    ...
    const ok = await sendWithRetry(inbox, lead, template);
